...
An ongoing experiment to compare array virtualization in GDAL and in xarray/VirtualiZarr. This repo exists for different ways of connecting to a virtualized-netcdf store, with various software tools. 

## A virtualized store

In directory [zarr/](https://github.com/mdsumner/virtualized/tree/main/zarr/ocean_temp_2023.zarr) there is a nested Parquet representation of a modelled ocean variable `ocean_temp`, this created by R and rhdf5 and GDAL multidim. 


In directory [remote/](https://github.com/mdsumner/virtualized/tree/main/remote/ocean_temp_2023.parq) there is a nested Parquet representation of a modelled ocean variable `ocean_temp`, this created by Python with VirtualiZarr. 
...
The script to do the raw processing in-situ for zarr/ is in R/nci. This relies on a custom build of rhdf5 (submitted as a PR to a future version)

The script to do the raw processing in-situ for remote/ is in py/ here. For some reason I am not seeing any benefit from parallelizing with concurrent.futures or with dask, it actually slows it down.  
...
Be sure to tone down concurrency before reading actual data (8 seems safe for thredds), not sure if zarr or dask is the best place 
